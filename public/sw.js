const CACHE='innova-emergencias-shell-v5';
...
self.addEventListener('install',event=>{
  event.waitUntil(caches.open(CACHE).then(async c=>{
    for(const path of SHELL){try{const r=await fetch(path,{cache:'reload'});if(r.ok&&String(r.headers.get('content-type')||'').includes('text/html')||path.endsWith('.webmanifest'))await c.put(path,r.clone())}catch{}}
  }));
  self.skipWaiting();
});
...
self.addEventListener('activate',event=>{
  event.waitUntil(Promise.all([
    self.clients.claim(),
    caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('innova-emergencias-')&&!([CACHE,MAP_CACHE].includes(k))).map(k=>caches.delete(k))))
  ]));
});
...
self.addEventListener('fetch',event=>{
  ...
  // Los bundles versionados de Next.js NO se interceptan. Evita JS antiguo después de redeploys.
  ...
  if(u.hostname.includes('openfreemap.org')||u.hostname==='tile.openstreetmap.org'){
    event.respondWith(caches.open(MAP_CACHE).then(async c=>{
      const cached=await c.match(event.request);
      const network=fetch(event.request).then(r=>{if(r.ok)c.put(event.request,r.clone());return r}).catch(()=>cached);
  ...
  if(event.request.mode==='navigate'){
    event.respondWith((async()=>{
      try{
        ...
        if(r.ok&&String(r.headers.get('content-type')||'').includes('text/html'))caches.open(CACHE).then(c=>c.put(event.request,r.clone()));
        return r;
      }catch{
...
async function createOrConfirm(r){
  let last=null;
  for(let attempt=0;attempt<2;attempt++){
    try{
      const res=await fetch('/api/reports',{method:'POST',headers:{'content-type':'application/json','cache-control':'no-cache'},body:payloadFor(r),cache:'no-store'});
      const text=await res.text();let data={};try{data=text?JSON.parse(text):{}}catch{}
      if(res.ok)return data;
      last=new Error(data.error||`No se pudo registrar (${res.status})`);
      if(res.status<500&&res.status!==408&&res.status!==429)break;
    }catch(e){last=e instanceof Error?e:new Error('Error de conexión')}
    if(attempt===0)await sleep(700);
  }
  throw last||new Error('No se pudo confirmar recepción');
}
...
async function syncOne(r){
  ...
  try{
    if(!r.serverAccepted){
      const created=await createOrConfirm(r);
      r.serverAccepted=true;r.publicCode=created.publicCode||r.publicCode;r.incidentId=created.incidentId||r.incidentId;r.acceptedAt=r.acceptedAt||new Date().toISOString();r.state='accepted';await putReport(r);
      await notify({type:'REPORT_ACCEPTED',id:r.id,publicCode:r.publicCode||null});
    }
    for(const e of r.evidence||[]){
      if(e.uploaded)continue;
      const sign=await fetch(`/api/reports/${r.id}/evidence-url`,{method:'POST',headers:{'content-type':'application/json','cache-control':'no-cache'},cache:'no-store',body:JSON.stringify({secret:r.secret,fileName:e.name,mimeType:e.type,bytes:e.size,mediaType:e.mediaType,durationSeconds:e.durationSeconds})});
      const s=await sign.json().catch(()=>({}));if(!sign.ok||!s.signedUrl||!s.path)throw new Error(s.error||'No se pudo preparar evidencia');
      const up=await fetch(s.signedUrl,{method:'PUT',body:e.blob,headers:{'Content-Type':e.type||'application/octet-stream','x-upsert':'false'}});if(!up.ok)throw new Error(`Evidencia pendiente (${up.status})`);
      const confirm=await fetch(`/api/reports/${r.id}/evidence-confirm`,{method:'POST',headers:{'content-type':'application/json','cache-control':'no-cache'},cache:'no-store',body:JSON.stringify({secret:r.secret,storagePath:s.path,mimeType:e.type,bytes:e.size,mediaType:e.mediaType,durationSeconds:e.durationSeconds})});if(!confirm.ok)throw new Error('Falta confirmar evidencia');
      e.uploaded=true;e.storagePath=s.path;await putReport(r);
    }
    const code=r.publicCode||null;await delReport(r.id);await notify({type:'REPORT_SYNCED',id:r.id,publicCode:code});
  }catch(e){r.state=r.serverAccepted?'accepted':'failed';r.lastError=r.serverAccepted?`Emergencia recibida. Pendiente: ${String(e&&e.message||e)}`:String(e&&e.message||e);await putReport(r)}
}

async function runSync(){const rs=await allReports();for(const r of rs)await syncOne(r)}
self.addEventListener('sync',event=>{if(event.tag==='sync-emergencies')event.waitUntil(runSync())});
self.addEventListener('message',event=>{if(event.data==='SYNC_NOW'||event.data&&event.data.type==='SYNC_NOW')event.waitUntil(runSync())});
