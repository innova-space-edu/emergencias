...
export async function POST(req:NextRequest){
  try{
    ...
    const admin=process.env.ADMIN_EMAIL||'contacto@innova-space-edu.cl';
    try{await sendInstitutionalMail({to:admin,subject:`Solicitud de acceso — ${String(body.organization||'Institución')}`,text:`Nueva solicitud de acceso a Innova Emergencias\n\nNombre: ${body.fullName||''}\nCorreo: ${body.email||''}\nInstitución: ${body.organization||''}\nCargo: ${body.position||''}\nRol solicitado: ${body.requestedRole||''}\n\n${body.message||''}`})}catch(error){console.warn('Solicitud guardada, correo no enviado',error)}
