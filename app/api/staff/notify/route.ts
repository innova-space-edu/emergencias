...
export async function POST(req:NextRequest){
  ...
  try{
    ...
    if(channel==='email'&&destination.includes('@')){
      ...
      const cc=destination.toLowerCase()===admin.toLowerCase()?undefined:[admin];
      try{
        const sent=await sendInstitutionalMail({to:destination,cc,replyTo:staff.user.email||staff.profile.email||admin,idempotencyKey:`notify-${incidentId}-${organizationId||organizationName}-${Date.now().toString().slice(0,-4)}`,subject:`Alerta informativa ${incident.public_code} — Innova Emergency`,text:`Innova Emergency canaliza un reporte ciudadano a solicitud de un usuario autorizado. Esta comunicación no confirma que el hecho sea verdadero y no constituye una orden de despacho.\n\nCódigo: ${incident.public_code}\nTipo: ${incident.title||incident.category}\nEstado: ${incident.status}\nComuna: ${incident.commune||'No indicada'}\nLocalidad: ${incident.locality||'No indicada'}\nReferencia: ${incident.address_approx||'Ubicación registrada'}\nGPS: ${incident.latitude}, ${incident.longitude}\nResumen público: ${incident.public_summary||'En revisión'}\n\nGestionado por: ${staff.user.email||staff.profile.email||'usuario autorizado'}\n\nEsta plataforma complementa la comunicación ciudadana y no reemplaza 131 SAMU, 132 Bomberos, 133 Carabineros ni SAE/SENAPRED.`});
        providerMessageId=sent.id;status='sent';
      }catch(e){status='failed';failureReason=e instanceof Error?e.message:'Fallo de correo'}
    }
    const {data,error}=await s.from('incident_notifications').insert({incident_id:incidentId,organization_id:organizationId,organization_name:organizationName,channel,destination:destination||null,status,provider_message_id:providerMessageId,sent_at:status==='sent'?new Date().toISOString():null,failure_reason:failureReason,created_by:staff.user.id}).select('*').single();if(error)throw error;
