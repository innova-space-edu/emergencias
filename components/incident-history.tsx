...
import { EMERGENCY_CATEGORIES, STATUS_COLOR, STATUS_LABEL } from '@/lib/constants';
import { groupByHourAndCategory } from '@/lib/emergency-grouping';

type Filters={code:string;commune:string;locality:string;status:string;category:string;from:string;to:string};
const EMPTY:Filters={code:'',commune:'',locality:'',status:'',category:'',from:'',to:''};
const PAGE_SIZE=50;
const CATEGORY_LABEL=Object.fromEntries(EMERGENCY_CATEGORIES) as Record<string,string>;
...
export default function IncidentHistory(){
  ...
  const grouped=useMemo(()=>groupByHourAndCategory(rows,'last_reported_at'),[rows]);
  ...
  return <div className="content-shell ops-page history-shell">
    <div className="page-heading"><span className="eyebrow">ARCHIVO OPERATIVO</span><h1>Historial general de emergencias</h1><p>Organizado por hora y, dentro de cada hora, por categoría. Las emergencias más prioritarias aparecen primero en cada grupo.</p></div>
    <form className="history-filters panel-card" onSubmit={submit}>
      ...
      <label>Categoría<select value={draft.category} onChange={e=>setDraft({...draft,category:e.target.value})}><option value="">Todas</option>{EMERGENCY_CATEGORIES.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>
    ...
    <div className="history-list history-grouped-list">
      {loading?<div className="panel-card">Cargando historial…</div>:rows.length===0?<div className="panel-card muted">No hay emergencias con esos filtros.</div>:grouped.map((hour,hIndex)=><details className="history-hour-group" key={hour.key} open={hIndex<2}><summary><div><b>{hour.label}</b><small>{hour.count} emergencia(s)</small></div><span>Por categorías</span></summary>{hour.categories.map(cat=><details className="history-category-group" key={`${hour.key}-${cat.category}`} open={hIndex===0}><summary><span>{CATEGORY_LABEL[cat.category]||cat.category}</span><b>{cat.count}</b></summary>{cat.rows.map((r:any)=><article className="history-row" key={r.id}>
        <div className="history-state"><i style={{background:STATUS_COLOR[r.status]||'#64748b'}}/><span>{STATUS_LABEL[r.status]||r.status}</span></div>
        <div className="history-main"><div><b>{r.public_code}</b><strong>{r.title||CATEGORY_LABEL[r.category]||r.category}</strong></div><p>{r.address_approx||r.locality||r.commune||'Ubicación registrada'}</p><small>P{r.severity} · {r.commune||'—'}{r.locality?` · ${r.locality}`:''} · {r.reports_count||0} reporte(s)</small></div>
