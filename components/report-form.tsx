...
export default function ReportForm(){
 ...
 setSending(true);await saveOfflineReport(report);await refreshPending();setSavedCode(`LOCAL-${id.slice(0,8).toUpperCase()}`);setStatus(navigator.onLine?'Reporte guardado. Confirmando recepción…':'Reporte guardado en este dispositivo. Se enviará cuando vuelva Internet.');
 try{
   if(navigator.onLine){const synced=await syncOneReport(report);setSavedCode(synced.publicCode||report.publicCode||`LOCAL-${id.slice(0,8).toUpperCase()}`);setStatus('Emergencia recibida correctamente por la plataforma.');await refreshPending();setFiles([]);form.reset();setLocality('');setAddressApprox('');touched.current={region:false,commune:false,locality:false,addressApprox:false};}
   else await requestBackgroundSync().catch(()=>{});
 }catch(error){
   await requestBackgroundSync().catch(()=>{});const syncError=error as Error & {serverAccepted?:boolean;publicCode?:string};const accepted=Boolean(syncError.serverAccepted||report.serverAccepted);if(accepted){setSavedCode(syncError.publicCode||report.publicCode||savedCode);setStatus(report.evidence.some(x=>!x.uploaded)?'Emergencia recibida correctamente. La evidencia restante seguirá cargándose automáticamente.':'Emergencia recibida correctamente por la plataforma.');}else{setStatus('Aún no fue posible confirmar la recepción. El reporte permanece guardado en este dispositivo y se reintentará automáticamente.');}await refreshPending();
