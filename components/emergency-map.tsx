...
function pointColor(severity:number){return severity>=5?'#991b1b':severity>=3?'#ef4444':'#f59e0b'}
...
export default function EmergencyMap({mode='public',onSelect}:{mode?:Mode;onSelect?:(id:string)=>void}){
 ...
 const load=useCallback(async()=>{try{const r=await fetch('/api/public/incidents',{cache:'no-store'});const j=await r.json();if(r.ok&&Array.isArray(j.incidents))setIncidents(mode==='preview'?j.incidents.slice(0,100):j.incidents.slice(0,600))}finally{setLoading(false)}},[mode]);
 ...
 useEffect(()=>{
   ...
   let alive=true;(async()=>{const L=await import('leaflet');if(!alive)return;group.clearLayers();for(const i of incidents){if(!Number.isFinite(i.latitude)||!Number.isFinite(i.longitude))continue;const marker=L.circleMarker([i.latitude,i.longitude],{radius:i.severity>=5?11:i.severity>=3?9:8,color:'#fff',weight:3,fillColor:pointColor(i.severity),fillOpacity:1});marker.bindTooltip(`${i.public_code} · ${categoryLabel(i.category)}`,{direction:'top'});marker.on('click',()=>{setSelected(i.id);setPanelOpen(true);onSelect?.(i.id)});group.addLayer(marker)}})().catch(()=>setMapUnavailable(true));return()=>{alive=false};
 },[incidents,onSelect]);
