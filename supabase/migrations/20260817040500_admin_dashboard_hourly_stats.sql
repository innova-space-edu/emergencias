...
begin
  ...
  select jsonb_build_object(
    ...
    'hours_24', (
      select coalesce(jsonb_agg(jsonb_build_object('hour',hour_label,'category',category,'count',cnt) order by hour_value desc, cnt desc),'[]'::jsonb)
      from (
        select date_trunc('hour', last_reported_at at time zone 'America/Santiago') hour_value,
               to_char(date_trunc('hour', last_reported_at at time zone 'America/Santiago'),'YYYY-MM-DD"T"HH24') hour_label,
               category,
               count(*)::int cnt
        from public.incidents
        where last_reported_at >= now() - interval '24 hours'
        group by 1,2,3
      ) h
    )
  ) into result;
