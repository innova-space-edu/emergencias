...
async function assess(sb:any,body:any){
 ...
 const exceedsAutoSeverity=assessment.severity>Number(policy.max_severity_auto||3);
 const autoTriageDisabled=!Boolean(policy.auto_triage_enabled);
 const requiresHuman=Boolean(assessment.needsHumanReview||assessment.inconsistencies.length||categoryConflict||danger||assessment.severity>=4||exceedsAutoSeverity||autoTriageDisabled||assessment.confidence<Number(policy.min_confidence_auto||.9));
 ...
 if(requiresHuman)actions.push({run_id:run.id,incident_id:incident.id,action_type:'human_review',status:'requires_approval',payload:{priority:decision==='urgent_human_review'?'urgent':'normal',reason:assessment.reason,policy:{autoTriageEnabled:Boolean(policy.auto_triage_enabled),maxAutoSeverity:Number(policy.max_severity_auto||3),minConfidence:Number(policy.min_confidence_auto||.9)}}});
